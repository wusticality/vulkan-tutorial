use crate::{
    Device, HostBuffer, Pipeline, PipelineSettings, RenderPass, StagedStaticBuffer, Swapchain,
    VertexDescriptions
};
use anyhow::Result;
use ash::vk::{self};
...
struct PerFrameData {
    /// The uniform buffer.
    uniforms: HostBuffer<UniformData>,
...
pub struct TriangleRenderer {
    ...
    /// The vertex buffer.
    vertices: StagedStaticBuffer,

    /// The index buffer.
    indices: StagedStaticBuffer,
...
impl TriangleRenderer {
    pub unsafe fn new(
        ...
        let vertices =
            StagedStaticBuffer::new(device, vk::BufferUsageFlags::VERTEX_BUFFER, &VERTICES)?;

        // Create the index buffer.
        let indices =
            StagedStaticBuffer::new(device, vk::BufferUsageFlags::INDEX_BUFFER, &INDICES)?;
        ...
        let per_frame_data = (0..frames_in_flight)
            .map(|_| PerFrameData::new(&device, &descriptor_pool, &descriptor_set_layout))
